...
We decided to use multilevel logistic regression to model the proportion of voters who will vote for Donald Trump. In our model, we are using a variety of factors to model the probability of Donald Trump being voted for. The data for the model was taken from the Democracy Fund + UCLA Nationscape ‘Full Data Set’, which is an individual-level survey of *6479* observations of *18* categories (Democracy Fund Voter Study Group, 2020). To create a meaningful model for our prediction, we took the explanatory variables to be:  
1. Gender\newline
2. Age\newline
3. State\newline
4. Education\newline
...
A *two-level* logistic regression model given by:
...
The reason why we set categories in these variables will be explained in more detail in the post-stratification section.
```{r, echo = FALSE}
...
According to our calculations, we estimated the proportion of voters to be `r y_s`. Meaning approximately `r 100*round(y_s, 4)`% of people will vote for Donald Trump. By means of post-stratification, we calculated this number as the proportion of voters in favor of Donald J. Trump, modeled by a multilinear logistic regression. Our model which accounted for gender, age, race, education, and employment status gives us a clear answer in what we are searching for.

## Descriptive Analysis about Prediction Results

We are also interested in predicting how different states behave in voting, this can be seen in Graph 1. By setting the population as each state, we can use the same formula mentioned before to do prediction.  
...
ggplot(state_prediction, aes(x=state, y=Prediction, group = 1)) +
  ...
  ggtitle("Graph 1: Predicted Trump Vote based on State")
```
Using similar methods as discussed above, we can present a graph on the predicted votes for Donald Trump based on age (Graph 2). 

```{r, echo=FALSE, message=FALSE, warning=FALSE}

ggplot(age_prediction, aes(x=age_group, y=Prediction, group = 1)) +
  geom_line() + geom_hline(yintercept = 0.5, linetype = "dashed", color = "red")+
  ggtitle("Graph 2: Predicted Trump Vote based on Age")+
  labs(x="Age Group")
```
...
To be able to predict the outcome of the 2020 American federal election, our group performed a post-stratification calculation using census data, based on a multilevel logistic model from the Democracy Fund + UCLA Nationscape sample data. By eliminating results in the "vote 2020" category from the survey, we created a binomial response using either "Donald Trump" or "Joe Biden". For the model we saw that gender can be treated as a fixed observation (since it is independent of other categories), while the rest of the observations (age, education, race, employment, state) are considered dependent, therefore requiring multilevel regression modeling.  

After splitting the data into cells and taking weighted average of estimates within each cell, post-stratification using the data from the census survey provided us with an estimate for the proportion of all people that will be voting for Donald Trump. We found this number to be `r 100*round(y_s, 4)`%. \newline

We also used descriptive analyses for predicting the election based on state, and age (Graphs 1 & 2 respectively). From the first graph, we can see that individual have very contrasting views due to the variance of results. We can say the highest probability to vote for Donald Trump are Idaho, Kansas, and Tennessee. While the states with highest probability to vote Joe Biden are Washington DC, California, and Massachusetts. The reason these states have such a large probability for their respective candidates could be due to state influence (depending on which is Republican or Democrat leaning).  

When looking at Graph 2, analysis for prediction based on age shows that most young adults are likely to be in favor of Joe Biden. However there is a strongly increasing trend for Donald Trump in relation to increasing age, peaking at ages 45-60, then dropping off thereafter.\newline

## Conclusion

In conclusion we can say that `r 100*round(y_s, 4)`% of people will be voting for Donald Trump, therefore Joe Biden is expected to win the election by a narrow margin (`r 100*round(1-y_s, 4)`%). Given age, and state, we can also see trends supporting the hypothesis that Donald Trump will lose. Both graphs show favor for Joe Biden, along with more supportive evidence for other observations that can be seen in graphs 3-6 in the appendix.
...
ggplot(race_prediction,aes(x=race_group, y=Prediction, group = 1)) +
  geom_line() + geom_hline(yintercept = 0.5, linetype = "dashed", color = "red")+
  ggtitle("Graph 3: Predicted Trump Vote based on Race")+
  labs(x="Racial Group")+
  scale_x_discrete(guide = guide_axis(n.dodge = 2))

ggplot(gender_prediction,aes(x=gender, y=Prediction, group = 1)) +
  geom_line() + 
  geom_hline(yintercept = 0.5, linetype = "dashed", color = "red")+
  ggtitle("Graph 4: Predicted Trump Vote based on Gender")

ggplot(education_prediction,aes(x=education, y=Prediction, group = 1)) +
  geom_line() +
  geom_hline(yintercept = 0.5, linetype = "dashed", color = "red")+
  ggtitle("Graph 5: Predicted Trump Vote based on Education")+
  scale_x_discrete(guide = guide_axis(n.dodge = 4))

ggplot(employment_prediction, aes(x=employment_status, y=Prediction, group = 1)) +
  geom_line() +
  geom_hline(yintercept = 0.5, linetype = "dashed", color = "red")+
  ggtitle("Graph 6: Predicted Trump Vote based on Employment Status")+
